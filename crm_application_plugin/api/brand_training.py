    ...
    if type == "training":
        ...
        try:
            ...
            brand_info = {
                ...
                "pdfs": list(
                    map(
                        lambda x: {"name": x["title"] or "", "file": x["attach"] or ""},
            ...
            create_response(
                200, "Brand training details fetched successfully.", brand_info
            )
            return
        ...
        try:
            brand_info = {"brand_name": "", "pdfs": [], "videos": []}
            brand_pdfs = frappe.db.sql(
                """
            select bp.title as name, bp.attach as file
            from `tabBrand Pdfs` bp where bp.parent = "User Guide" order by bp.idx
            """,
                as_dict=1,
            )
            ...
            from`tabBrand Videos` bv where bv.parent = "User Guide" order by bv.idx
            """,
                as_dict=1,
            )
...
def get_brand_images(brand):
    try:
        brand_images = frappe.db.sql(
            """
            SELECT bi.attach AS file
            FROM `tabBrand Images` bi
            WHERE bi.parent = %(brand)s
            ORDER BY bi.idx
            """,
            {"brand": brand},
            as_dict=1,
        ...
        for img in brand_images:
            if img.get("file"):
                img["file"] = frappe.utils.get_url(img["file"])

        create_response(200, "Brand Images Fetched Successfully", brand_images)
        return
    except Exception as e:
        frappe.log_error(message=str(e), title="Error in brand images records")
        create_response(
            406,
            "Something Went Wrong",
            "An error occurred while processing your request.",
