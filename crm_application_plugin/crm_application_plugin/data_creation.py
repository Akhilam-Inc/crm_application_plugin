...
def get_items_and_customers():
    ...
    customers = frappe.db.get_all("Customer" , fields = ['name'], page_length = 20)
...
def generate_order_data(num_orders=1):
    ...
    for _ in range(num_orders):
        ...
        items = [{"item_code": random.choice(item_code), "qty": random.randint(1, 3), "rate": round(random.uniform(150000.0, 500000.0),3),"custome_sales_person":erp_data["sales_person"]} for _ in range(items_count)]
    ...
    try:
        for invoice in invoices:
            try:
                ...
                frappe.log_error(title = "Sales Invoice creation error",message=frappe.get_traceback())
                frappe.db.commit()
    except Exception as e:
        frappe.log_error(title = "Sales Invoice Creation form Sale bot",message=frappe.get_traceback())
        frappe.db.commit()
...
def generate_invoice_data(num_invoices=1):
    ...
    sales_order_items = frappe.db.get_all("Sales Order Item" , fields = ['item_code' , 'qty' , 'rate' , 'parent','custom_sales_person'])
